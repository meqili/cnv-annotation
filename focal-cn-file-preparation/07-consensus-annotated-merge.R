...
consensus_x_and_y <- consensus_x_and_y %>% dplyr::select(-germline_sex_estimate)
combined <- rbind(consensus_auto, consensus_x_and_y)

# readr::write_tsv(combined, 
#           file.path(opt$outdir, "consensus_wgs_plus_cnvkit_wxs.tsv.gz"))
readr::write_tsv(combined, 
          file.path(opt$outdir, "consensus_annotated.tsv.gz"))
