...
```{r}
setwd("focal-cn-file-preparation")
...
consensus_seg_file <- file.path("..", "results", 
                                "BS_V6CNFCGS/cnv-consensus.seg.gz")
histologies_file <- file.path("..", "data", "histologies.txt")
...
add_xy_df <- add_ploidy_df %>%
  filter(chrom %in% c("chrX", "chrY")) %>%  
  mutate(status = case_when(
    germline_sex_estimate == "Male" & copy.num < 1 ~ "loss",
    germline_sex_estimate == "Male" & copy.num > 1 ~ "gain",
    germline_sex_estimate == "Male" & copy.num == 1 ~ "neutral",
    ...
    chrom == "chrX" & germline_sex_estimate == "Female" & copy.num < 2 ~ "loss",
    chrom == "chrX" & germline_sex_estimate == "Female" & copy.num > 2 ~ "gain",
    chrom == "chrX" & germline_sex_estimate == "Female" & copy.num == 2 ~ "neutral"
  ))
...
```{r}
output_file <- file.path("..", "results","BS_V6CNFCGS",  "consensus_seg_with_status.tsv")
